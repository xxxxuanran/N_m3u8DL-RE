...
从 [Releases](https://github.com/xxxxuanran/N_m3u8DL-RE/releases) 页面下载与系统匹配的压缩包。推送形如 `v0.6.2-beta` 的 Git 标签后，GitHub Actions 会自动构建并发布各平台产物。
...
| Linux x64 (musl 静态) | `N_m3u8DL-RE_v0.6.2-beta_linux-x64_*.tar.gz` |
| Linux arm64 (musl 静态) | `N_m3u8DL-RE_v0.6.2-beta_linux-arm64_*.tar.gz` |
...
### Live from start（从直播起点回溯录制）

`--live-from-start` 让你在录制直播实时内容的同时，把开始录制点**之前、服务器 DVR 窗口里仍可获取的较早分片**也尽量补齐，使最终文件的起点尽可能靠近本场直播的开头。

> [!NOTE]
> 仅适用于**分片文件名为连续递增数字**且 URL 规律可预测的直播（如 B 站 fmp4 流）。无法预测命名时会自动跳过回溯，仅录制实时内容。

#### 核心难点

DVR 窗口的最旧分片**正以真实时间的速度被服务器删除**（过期边界持续上移）。回溯本质是一场与过期边界的赛跑：下载顺序不当或吞吐不足，最早的分片会在被取到之前消失。此外窗口最旧端往往**参差不齐**（老分片正被逐个轮转，可用/缺失交替），简单的"单调可用"假设并不成立。

#### 工作流程

```mermaid
flowchart TD
    A["开始录制点 S<br/>(实时录制照常进行)"] --> B["阶段1: 指数探测 + 二分<br/>对数次请求内定位最早可用边界 E0"]
    B --> C{"可用区是否很浅?<br/>(深度 ≤ ~60，约1分钟)"}
    C -->|"是 (如刚开播)"| D["倒序方案<br/>从 S-1 向下全量竞速<br/>遇首个不可得即停"]
    C -->|"否 (深 DVR)"| E["升序回填<br/>从 E0 起按编号递增并发下载<br/>抢在过期边界前面"]
    E --> F["参差尾部裁剪<br/>以最高不可补齐空洞为界<br/>只保留连接直播边缘的连续段"]
    D --> G["合并: 连续分片接到直播边缘"]
    F --> G
```

- **阶段 1 — 定位最早可用边界**：用指数探测（步长翻倍）加二分查找，仅需约 `2·log₂(D)` 次请求即可锁定 DVR 窗口的最早可用分片 `E0`，无需逐个试探。
- **阶段 2 — 回填**：
  - **深 DVR**：从 `E0` **升序**（编号由小到大）并发回填，始终优先抢救最旧、最临近过期的分片，从结构上消除"追得慢丢早期分片"。完成后以"最高的不可补齐空洞"为界裁剪，**只保留能连续拼接到直播边缘的那一段**（参差尾部里孤立存活、无法连续利用的碎片会被丢弃）。
  - **浅可用区（如刚开播、只缺开头一分钟以内）**：改用**倒序**方案，从 `S-1` 向下下载、遇到第一个确认不可得的分片即停，天然得到连接直播边缘的连续段。区间短，几乎不受过期影响。

#### 与镜像协同（`--live-host-mirror`）
